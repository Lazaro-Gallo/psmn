...
/**
 * Description of DevolutiveParte4MPE
 *
 * @author vtx
 */
class Vtx_Devolutive_Tipo_MPE_DevolutiveParte4MPE {
    ...
    public function introducaoParte4()
    {
        ####################################
        //pagina 1 do bloco
        $this->objMakePdf->AddPage();

        $intro_parte4 = 'INOVACAO-CAPA.pdf';
        $pags = array($intro_parte4);
        $insertPdf = new Vtx_Devolutive_Tipo_MPE_InsertPdf($pags, $this->objMakePdf);                
        
        
        $this->objMakePdf->AddPage();
        
        $intro_parte4 = 'Parte4_PraticasInovacao.pdf';
        $pags = array($intro_parte4);
        $insertPdf = new Vtx_Devolutive_Tipo_MPE_InsertPdf($pags, $this->objMakePdf);         
        //$this->objMakePdf->MultiCell(190,8,utf8_decode("contra capa Parte 2 da Devolutiva "),0,"J",true);
        
    }


    
    public function DevolutiveParte4MPE($arrDevolutive, $arrBlocks, $arrCriteria, $offSet = 0, $isRA = false) 
    {

        //printa capa e contracapa
        $this->introducaoParte4();
        ...
        $this->objMakePdf->pagina = 76; //num inicial da questao deste bloco, para pegar num pdf de fundo
        #####################################
                ...
                foreach ($question['alternatives'] AS $alt_designation => $alt_value) {
                    ...
                    if (isset($alt_alternative_designation) && $alt_designation == $alt_alternative_designation) 
                    {
                        ...
                        switch ($alt_text_letra) {
                            case 'A':
                                $this->objMakePdf->SetXY(25,120);
                                break;
                            case 'B':
                                $this->objMakePdf->SetXY(25,140);
                                break;
                            case 'C':
                                $this->objMakePdf->SetXY(25,161);
                                break;
                            case 'D':
                                $this->objMakePdf->SetXY(25,184);
                                break;
