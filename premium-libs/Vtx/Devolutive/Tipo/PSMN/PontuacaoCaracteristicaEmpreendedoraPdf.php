 ...
 * @author esilva
 */
class Vtx_Devolutive_Tipo_PSMN_PontuacaoCaracteristicaEmpreendedoraPdf
{
    ...
    {        
        ...
        $objPontuacao = $grade->getAll( array('UserId = ?' => $this->objDevolutive->getUserId(),
                                              'QuestionnaireId = ?' => $this->objDevolutive->getQuestionnaireId(),
                                              'BlockId = ?'=> $this->objDevolutive->getBlockIdEmpreendedorismo(),
    ...
    public function FancyTable($header, $data)
    {
        ...
        foreach($data as $row)
        {
            ...
            $this->objMakePdf->Cell($w[2],6,$coluna3,'LR',0,'C',$fill);
