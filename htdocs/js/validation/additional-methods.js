...
jQuery.validator.addMethod("dateITA", function(value, element) {
    ...
    var date = {};
    if( re.test(value)){
    	var value_parsed = value.match(re);
    	if (value_parsed) {
    		date.parsed = {
				day: parseInt(value_parsed[1], 10),
				month: parseInt(value_parsed[2], 10),
				year: parseInt(value_parsed[3], 10)
    		};
			date.parsed.isLeap = (((date.parsed.year % 4) === 0) && (((date.parsed.year % 100) !== 0) || ((date.parsed.year % 400) === 0)));
    		switch (true) {
    			case (((date.parsed.day <= 0) || (date.parsed.day > 31)) || ((date.parsed.month <= 0) || (date.parsed.month > 12)) || (date.parsed.year <= 0)):
    				precheck = false;
    				break;
    			case (date.parsed.month === 2):
    				precheck = ((date.parsed.day > 29) ? false : ((date.parsed.day === 29) ? date.parsed.isLeap : true));
    				break;
    			case ((date.parsed.month === 4) || (date.parsed.month === 6) || (date.parsed.month === 9) || (date.parsed.month === 11)):
    				precheck = ((date.parsed.day > 30) ? false : true);
    				break;
    			default:
    				precheck = ((date.parsed.day > 31) ? false : true);
    				break;
    		}
    		if (precheck) {
				var date = new Date(Date.UTC(
					parseInt(value_parsed[3], 10),
					(parseInt(value_parsed[2], 10) - 1),
					(parseInt(value_parsed[1], 10) + 1),
					0,
					0,
					0,
					0
