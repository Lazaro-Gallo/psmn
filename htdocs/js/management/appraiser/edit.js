...
var appraiserModule = (function () {
    ...
    checkChange = function(){
        ...
        $.ajax({
            url: BASE_URL + '/management/appraiser/set-appraiser-to-enterprise/format/json',
            type: 'post',
            dataType: 'json',
            data: {
                enterpriseId: $enterpriseId, appraiserId: $('#AppraiserId').val(), checked: $isChecked
            },
            success: function(json, $statusText) {
                if (!($statusText == 'success') || !json.itemSuccess) {
                    alert('erro');
