...
class Management_DevolutiveVerificadorController extends Vtx_Action_Abstract
{
    ...
    {   
        ...
        $this->Devolutive = new Model_DevolutiveVerificador();
    ...
    public function indexAction()
    {
       ...
       if ( isset($limit) && isset($enterpriseProgramaIdMaiorQue) ) {
        ...
        $this->Devolutive->setUserId($user_id);
		 $this->Devolutive->setEnterpriseKey( $enterprise);
        ...
        $geraProtocolo = $this->modelProtocolo->geracaoDoProtocolo( $this->view, $this->Devolutive, $this->Execution ,new Model_User(), $questionnaire_id, 
                                                                    $user_id, $this->loggedUserId, $competitionId , true
                                                               );
																   
		
		$this->Devolutive->setProtocolo($this->view->protocolo);
		$this->Devolutive->setProtocoloCreateAt($this->view->protocoloCreateAt);
		$this->Devolutive->setProtocoloIdDevolutiva($this->view->protocoloId);
 
		$this->Devolutive->setUserLogadoGerouDevolutiva($userLogged->getFirstName());
        ...
        */
        ...
        //var_dump($this->Devolutive->getDevolutiveId());
        $devolutivePath = $this->Devolutive->makePdfDevolutive();
		//var_dump($devolutivePath);
      ...
      //  $this->Devolutive->configuraGravaPontuacaoExecution($user_id);
		...
		$dirName = $this->Devolutive;
		//print_r( $devolutiveAlreadyExists);exit;
 /*
		 try {
        ...
        //if(!$devolutiveAlreadyExists){
            $pdf = new Report_Devolutive_PDF($this->Devolutive, APPLICATION_PATH.'/../htdocs'.$devolutivePath,4);
            $pdf->saveToFile();
			//var_dump( $pdf);
       // }
        ...
        if ($devolutivePath) {
			 ...
			 header('Location: '.$devolutivePath);
             return;
        }
    ...
    public function indexnacAction()
    {
    	...
    	$limit = $this->_getParam('limit');
    	$enterpriseProgramaIdMaiorQue = $this->_getParam('maiorque');
// Sandra - setar variável de nacional
    	$this->Devolutive->setNacional(true);
    	//mpe9 / mpe9
    	if ( isset($limit) && isset($enterpriseProgramaIdMaiorQue) ) {
